...
// NewReusingMetrics makes a new Server and attempts to reuse any metrics that have already been registered.
func NewReusingMetrics(cfg Config) (*Server, error) {
	metrics := newServerMetrics(cfg)
	metrics.mustRegisterOrReuse()
	return newServer(cfg, metrics)
}

func newServer(cfg Config, metrics *serverMetrics) (*Server, error) {
